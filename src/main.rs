...
#[structopt(name = "pcheck", about = "A tool to find single-precision floating point numbers inside of binary data.")]
struct Opt {
    #[structopt(parse(from_os_str), help = "The file to search.")]
    input: Option<PathBuf>,

    #[structopt(long, default_value = "1048576", conflicts_with("input"), help = "If no file provided, the number of floats to generate as sample data.")]
    sample_data_length: usize,

    #[structopt(long, help = "Minimum value of floats to search for (inclusive).")]
    min: Option<f32>,

    #[structopt(long, required_unless("min"), help = "Maximum value of floats to search for (inclusive).")]
    max: Option<f32>,
...
fn main() {
    ...
    let bytes = if let Some(path) = &opt.input {
        ...
        // Set up example data.
        println!("No input provided. Generating {} bytes of sample data.", 4*opt.sample_data_length);
        let mut rng = rand::thread_rng();
        let input: Vec<f32> = (0..opt.sample_data_length).map(|_| rng.gen_range(-100.0..1.0)).collect();
